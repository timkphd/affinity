...
subroutine dohelp
   ...
   write (*, *)
   write (*, *) " -i          : Report MPI_Init times for each task at the end of the run"
   write (*, *)
...
module dlaunch
double precision stl(2)
integer doinit
contains
subroutine treport()
  use mpi 

     double precision , allocatable :: allst(:)
     double precision  s,e
     integer numprocs,i,myid,ierr
     call MPI_Comm_size(MPI_COMM_WORLD, numprocs,ierr)
     call MPI_Comm_rank(MPI_COMM_WORLD, myid,ierr)
        allocate(allst(2*numprocs))
        call MPI_Gather(stl,2,MPI_DOUBLE_PRECISION, &
                        allst,2,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
                if (myid .eq. 0 .and. doinit .eq. 1)then
                do i=1, numprocs
                  s=allst((i)*2-1);
                  e=allst((i)*2);
                  write(*,"('mpi_init',i5,f20.4,f20.4,f12.4)")i-1,s,e,e-s
                end do
        end if
end subroutine
end module
